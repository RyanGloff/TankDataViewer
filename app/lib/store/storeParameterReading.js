...
export default function storeParameterReading(
  pgClient,
  tank_id,
  parameter_id,
  value,
...
INSERT INTO tank_data_schema.parameter_reading(tank_id, parameter_id, value, time) VALUES ($1, $2, $3, $4);
  ...
  return pgClient.query(sql, params).catch((err) => {
    ...
    console.warn(`Unknown error: ${err.detail}`);
    throw err;
  });
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(
    `Storing parameter with values: TankId -> ${tankId}, ParameterId -> ${parameterId}, Value -> ${value}, Time -> ${time.toISOString()}`,
  );
  usePgClient("postgres", (pgClient) =>
    storeParameterReading(pgClient, tankId, parameterId, value, time),
  );
}
